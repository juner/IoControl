...
namespace IoControl
{
    ...
    public class IoControl : IDisposable
    {
            ...
            where TINOUT : struct
        {
            ...
            using (InternalDisposable.Create(iogch.Free))
            {
            ...
            where TOUT : struct
        {
            ...
            using (InternalDisposable.Create(igch.Free))
            using (InternalDisposable.Create(ogch.Free))
            {
        ...
        public bool DeviceIoControlInOnly<TIN>(IOControlCode dwIoControlCode, ref TIN InBuffer, out uint ReturnBytes)
            where TIN : struct
        {
            var inSize = (uint)Marshal.SizeOf(typeof(TIN));
            var _inBuffer = new byte[inSize];
            ...
            where TOUT : struct
        {
            ...
            using (InternalDisposable.Create(() => Marshal.FreeCoTaskMem(outPtr)))
            {
        ...
        internal class InternalDisposable : IDisposable
        {
            Action Action;
            internal InternalDisposable(Action Action) => this.Action = Action;
            public static InternalDisposable Create(Action Action) => new InternalDisposable(Action);
            public void Dispose()
            {
                try
                {
